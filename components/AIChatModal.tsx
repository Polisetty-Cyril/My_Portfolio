  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl h-full max-h-[80vh] flex flex-col border border-stone-200"
      ...
      >
        ...
        <div ref={chatContainerRef} className="flex-grow p-4 overflow-y-auto space-y-4 bg-stone-50/50">
          {chatHistory.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              {msg.sender === 'model' && <div className="bg-amber-500 text-white p-2 rounded-full flex-shrink-0"><SparklesIcon /></div>}
              <div className={`p-3 rounded-xl max-w-md ${msg.sender === 'user' ? 'bg-stone-200' : 'bg-stone-800 text-white'}`}>
                <p className="whitespace-pre-wrap">{msg.text}</p>
              </div>
              {msg.sender === 'user' && <div className="bg-stone-200 text-stone-600 p-2 rounded-full flex-shrink-0"><UserIcon /></div>}
            </div>
          ))}
          {isLoading && (
            <div className="flex items-start gap-3">
              <div className="bg-amber-500 text-white p-2 rounded-full"><SparklesIcon /></div>
           ...
           {chatHistory.length === 0 && (
             <div className="text-center text-stone-500 p-8">
                <p className="mb-4">You can ask about my skills, experience, or career goals. Try one of these prompts:</p>
                <div className="flex flex-wrap justify-center gap-2">
                  {PORTFOLIO_DATA.aiChat.starterPrompts.map((prompt, i) => (
                     <button key={i} onClick={() => handleSubmit(prompt)} className="bg-stone-100 hover:bg-stone-200 text-sm px-3 py-1 rounded-full transition-colors border border-stone-200">{prompt}</button>
                  ))}
                </div>
             </div>
           )}
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
            <div className="relative">
              <input
                ...
                className="w-full bg-stone-100 border border-stone-300 rounded-full p-3 pl-4 pr-12 focus:outline-none focus:ring-2 focus:ring-amber-500"
                disabled={isLoading}
              />
              <button type="submit" disabled={isLoading || !userInput} className="absolute right-2 top-1/2 -translate-y-1/2 bg-stone-800 text-white p-2 rounded-full disabled:bg-stone-400 hover:bg-stone-900 transition-colors">
